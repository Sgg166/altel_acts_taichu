void Match(const std::string& rootFilePath){
  ...
  TH1F *hResidualU_det32_other = new TH1F("hResidualU_det32_other", "Residual U (det32, numMatched>2);u_{meas}-u_{fit} [um];Entries [50bin]", 50, -100, 100);
  TH1F *hResidualV_det32_other = new TH1F("hResidualV_det32_other", "Residual V (det32, numMatched>2);v_{meas}-v_{fit} [um];Entries [50bin]", 50, -100, 100);

  TH1F *hClusterWidthU_det32 = new TH1F("hClusterWidthU_det32", "Cluster Width  in U direction (det32); Cluster Width_U ;Entries", 20, 0, 10);
  TH1F *hClusterWidthV_det32 = new TH1F("hClusterWidthV_det32", "Cluster Height in V direction (det32); Cluster Width_V ;Entries", 20, 0, 10);
 ...
 /* if(hNumMatchedHits_det32->GetEntries() > 0){
  ...
  hNumMatchedHits_det32->GetYaxis()->SetTitle("Entries");
...
//  hNumMatchedHits_det32->SetMaximum(1.0);
  ...
  TF1 *fitU1 = new TF1("fitU1", "gaus", -30, 30);
  ...
  latex1.DrawLatex(0.55, 0.80, Form("Mean = %.2f", fitU1->GetParameter(1)));
  latex1.DrawLatex(0.55, 0.70, Form("Sigma = %.2f", fitU1->GetParameter(2)));
  ...
  latex2.DrawLatex(0.55, 0.80, Form("Mean = %.2f", fitV1->GetParameter(1)));
  latex2.DrawLatex(0.55, 0.70, Form("Sigma = %.2f", fitV1->GetParameter(2)));  
  ...
  latex3.DrawLatex(0.55, 0.80, Form("Mean = %.2f", fitU2->GetParameter(1)));
  latex3.DrawLatex(0.55, 0.70, Form("Sigma = %.2f", fitU2->GetParameter(2)));
  ...
  latex4.DrawLatex(0.55, 0.80, Form("Mean = %.2f", fitV2->GetParameter(1)));
  latex4.DrawLatex(0.55, 0.70, Form("Sigma = %.2f", fitV2->GetParameter(2)));
  ...
  latex5.DrawLatex(0.55, 0.80, Form("Mean = %.2f", fitU3->GetParameter(1)));
  latex5.DrawLatex(0.55, 0.70, Form("Sigma = %.2f", fitU3->GetParameter(2)));
  ...
  latex6.DrawLatex(0.55, 0.80, Form("Mean = %.2f", fitV3->GetParameter(1)));
  latex6.DrawLatex(0.55, 0.70, Form("Sigma = %.2f", fitV3->GetParameter(2)));
  ...
  std::ofstream sigmaFile("/home/sungaige/A/altel_acts/Draw/root_file/Match_sigma_output.txt");
  ...
  std::fprintf(stdout, "SigmaU1 = %.2f um, SigmaV1 = %.2f um\n", sigmaU1, sigmaV1);
  std::fprintf(stdout, "SigmaU2 = %.2f um, SigmaV2 = %.2f um\n", sigmaU2, sigmaV2);
  std::fprintf(stdout, "SigmaU3 = %.2f um, SigmaV3 = %.2f um\n", sigmaU3, sigmaV3);
...
//  c1->SaveAs("/home/pub/B/altel_acts/Draw/save_svg/match_hits_by_detector.svg");
 ...
 // hClusterWidthU_det32->SetLineColor(kBlack);
  ...
  if(hClusterWidthU_det32->GetEntries() > 0){
    hClusterWidthU_det32->Scale(1.0 / hClusterWidthU_det32->GetEntries());
  }
...
//  hClusterWidthV_det32->SetLineColor(kBlack);
  if(hClusterWidthV_det32->GetEntries() > 0){
    hClusterWidthV_det32->Scale(1.0 / hClusterWidthV_det32->GetEntries());
  }

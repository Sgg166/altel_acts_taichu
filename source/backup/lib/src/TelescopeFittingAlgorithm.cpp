...
ActsExamples::ProcessCode Telescope::TelescopeFittingAlgorithm::execute(
  ...
  if (!trackSourcelinks.empty()) {
    ...
    // @Below is what is used when the detector aligned along global x in the
    // first beginning. But this is not working when it's aligned along global
    // z. Needs investigation of the reason.
    // Acts::BoundSymMatrix cov;
    // cov << std::pow(10_mm, 2), 0., 0., 0., 0., 0., 0., std::pow(10_mm, 2),
    // 0.,
    //    0., 0., 0., 0., 0., 0.0001, 0., 0., 0., 0., 0., 0., 0.0001, 0., 0.,
    ...
    Acts::BoundSymMatrix cov;
    cov << resX2, 0., 0., 0., 0., 0., 0., resY2, 0., 0., 0., 0., 0., 0.,
        resPhi2, 0., 0., 0., 0., 0., 0., resTheta2, 0., 0., 0., 0., 0., 0.,
        0.0001, 0., 0., 0., 0., 0., 0., 1.;

    const Acts::Vector3D global0 =
        trackSourcelinks.at(0).globalPosition(ctx.geoContext);
    const Acts::Vector3D global1 =
        trackSourcelinks.at(1).globalPosition(ctx.geoContext);
    ...
    Acts::Vector3D rDir(sin(theta) * cos(phi), sin(theta) * sin(phi),
                        cos(theta));
    double q = 1;

    Acts::CurvilinearTrackParameters rStart(rPos4, rDir, q / m_cfg.beamEnergy,
                                            cov);
    // Acts::CurvilinearTrackParameters rStart(rPos4, phi, theta,
    // m_cfg.beamEnergy, q, cov);
...
namespace {
template <typename Fitter> struct FitterFunctionImpl {
  Fitter fitter;

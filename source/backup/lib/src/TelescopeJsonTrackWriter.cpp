...
Telescope::TelescopeJsonTrackWriter::TelescopeJsonTrackWriter(
  ...
  m_jsfp =
      std::fopen((m_cfg.outputDir + "/" + m_cfg.outputFileName).c_str(), "w");
  if (!m_jsfp) {
  ...
  m_jsos.reset(
      new rapidjson::FileWriteStream(m_jsfp, m_jsbuffer, sizeof(m_jsbuffer)));
  ...
  for (const auto &traj : trajectories) {
    ...
    for (const size_t &trackTip : trackTips) {
      ...
      mj.visitBackwards(trackTip, [&](const auto &state) {
        ...
        // std::cout<<"local position of hit on layer " << layerid<<" : " <<
        // local.x()<<", "<<local.y()<<std::endl;
        ...
        Acts::FreeVector freeParams =
            Acts::detail::transformBoundToFreeParameters(
                *state_surface, context.geoContext, state.smoothed());
        ...
        state_surface->initJacobianToGlobal(context.geoContext, jacToGlobal,

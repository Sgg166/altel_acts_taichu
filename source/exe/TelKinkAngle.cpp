...
// geometry closed, geometry ID only valid after geometry closing
  ...
  for(auto& [detId, aPlaneLayer]: mapDetId2PlaneLayer){
    ...
    else if(mapGeoId2DetId.at(aPlaneLayer->geometryId()) == 32 ){
  ...
  auto refSurface = Acts::Surface::makeShared<Acts::PlaneSurface>(
    Acts::Vector3D{0., 0., -10_m }, Acts::Vector3D{0., 0., 1.});
  ...
  TH1F *hkink_angle =  new TH1F("hkink_angle", " hkink_angle;kA_{dir_after-dir_before};Entries [100bin]", 50, 0, 0.005);

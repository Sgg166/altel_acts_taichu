...
#include "TelFragment_glsl.hh" // using Tel
  ...
  /// @see glm::perspective,  fovy degree
...
void TelGL::updateTransform(const JsonValue& js){
  ...
  Eigen::Vector3f eye_pos(js_eye["x"].GetDouble(), js_eye["y"].GetDouble(), js_eye["z"].GetDouble());
  Eigen::Vector3f object_center(js_center["x"].GetDouble(), js_center["y"].GetDouble(), js_center["z"].GetDouble());
  Eigen::Vector3f eye_up(js_up["x"].GetDouble(), js_up["y"].GetDouble(), js_up["z"].GetDouble());
  
  Eigen::Matrix4f view_matrix = lookAt(eye_pos, object_center, eye_up);
  ...
  Eigen::Matrix4f proj_matrix = perspective<float>(js_fov.GetDouble(), js_ratio.GetDouble(), js_near.GetDouble(), js_far.GetDouble());

  Eigen::Matrix<float,4, 12> mvp; //mvp[3]; // 0 model, 1 view, 2 proj
  mvp<< model_matrix, view_matrix, proj_matrix;
  
  glNamedBufferSubData(m_ubuffer_transform, 0, mvp.size()*sizeof(float), mvp.data());
}
